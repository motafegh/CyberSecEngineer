...
- [ ] Simulate failure and attack conditions, including concrete cloud attack paths.
...
Before this module, apply the Lab Extension Protocol from Phase 0 (Section 6) to build the container/Kubernetes zone.
...
### Real Cloud Attack Paths (LocalStack / Free-Tier Lab)

The exercises above teach misconfiguration concepts abstractly. This subsection adds concrete, named attack paths that map directly to real-world cloud incidents.

Core attack paths:

- SSRF to the instance metadata service, including the IMDSv2 token-based bypass discussion
- IAM privilege escalation through role chaining (`sts:AssumeRole` abuse)
- S3 (or equivalent object storage) bucket enumeration and unauthorized access
- Serverless function (Lambda or equivalent) over-permissioned execution role abuse

Hands-On Exercises:

- [ ] Stand up a LocalStack environment or use a free-tier cloud account with a strict budget alert configured.
- [ ] Deploy a vulnerable web service with SSRF and demonstrate retrieving instance metadata credentials.
- [ ] Demonstrate the difference between IMDSv1 (vulnerable to simple SSRF) and IMDSv2 (token-based, harder to abuse) and document why IMDSv2 mitigates basic SSRF-to-credential-theft.
- [ ] Create two IAM roles where Role A can assume Role B, and Role B has broader permissions than intended; demonstrate the escalation path.
- [ ] Create a publicly enumerable storage bucket naming pattern and demonstrate enumeration plus unauthorized object access.
- [ ] Create an over-permissioned serverless function execution role and demonstrate using the function's permissions beyond its intended scope.
- [ ] For each attack path, write the fix and re-test that the path is closed.
...
- [ ] Scanner results
- [ ] Risk-ranked report
- [ ] At least one real cloud attack path writeup (SSRF-to-metadata, IAM role chaining, S3 enumeration, or Lambda permission abuse) with reproduction steps, root cause, fix, and detection notes
...
- Why does IMDSv2 reduce, but not eliminate, the risk of SSRF-to-metadata attacks?
- Why does role chaining make IAM privilege escalation harder to spot than a single overbroad policy?
...
- [ ] Cloud metadata SSRF + Phase 2 SSRF exercises
...
- [ ] Explain the SSRF-to-metadata attack path end to end.
...
- [ ] Real cloud attack path exercises completed.
...
- [ ] HackTheBox Pro Lab intro sections (cloud/Kubernetes focused).
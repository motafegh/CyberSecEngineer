...
- [ ] Run a sustainable security research and field intelligence habit.
...
### Security-Specific Python Tooling

General Python fluency is not the same as security tooling fluency. This subsection focuses practice specifically on the patterns used in real assessment and defense work.

Core libraries:

- `requests` for HTTP-based recon and testing
- `scapy` for raw packet crafting and analysis
- `cryptography` for correct crypto primitive usage
- `argparse` for building reusable command-line tools with subcommands
- `subprocess` for safely wrapping external security tools

Hands-On Exercises:

- [ ] Build a small recon script using `requests` that fingerprints HTTP headers and server banners on a lab target.
- [ ] Build a packet crafting script with `scapy` that sends a custom TCP SYN and reads the response.
- [ ] Build a CLI tool using `argparse` with subcommands (e.g., `scan`, `report`).
- [ ] Wrap an external tool (e.g., `nmap` or `openssl`) safely with `subprocess`, avoiding shell injection.
- [ ] Use the `cryptography` library to correctly generate and verify a signature, contrasting with a deliberately broken example.
- [ ] Package one of the above as a reusable internal tool with a README, sample output, and a test.
...
- Why does wrapping an external tool with `subprocess` still require input validation even though the tool itself is trusted?
- Why is `argparse` with subcommands a better foundation for a security tool than a single flat script?

### Exit Criteria

- [ ] Python evidence collector works.
- [ ] Tool has README and sample output.
- [ ] At least one security-specific Python tool (recon script, packet crafting script, or wrapped external tool) works and is documented.
...
## 12. Module 1.9 — Security Research and Field Intelligence Habits

### Why This Matters

Security mechanisms age. A security engineer who stops reading the field falls behind within months: new CVEs, new exploitation techniques, new postmortems, new attacker tooling. This module is not theory; it builds a repeatable habit system that runs for the rest of your career, starting now.

### Core Concepts

- CVE / NVD tracking
- Vendor security advisories
- Public postmortems (Google Project Zero, Trail of Bits, Rekt.news for Web3)
- Threat actor / campaign reporting
- Spaced repetition applied to field reading
- Finding log vs mistake log

### Hands-On Exercises

Daily (10–15 minutes):

- [ ] Subscribe to a CVE/NVD feed relevant to your current stack.
- [ ] Subscribe to at least three vendor security advisory feeds (Linux distribution, a major cloud provider, a major framework you use).
- [ ] Skim daily security news from one trusted aggregator.

Weekly (30–60 minutes):

- [ ] Pick one public postmortem (Google Project Zero, Trail of Bits, a CNCF/Kubernetes security advisory, or — once Phase 4 begins — a Web3 postmortem from Rekt.news) and read it in full.
- [ ] Write a three-sentence summary: what failed, why it failed, what control would have stopped it.
- [ ] Log the postmortem in the finding log.

Monthly:

- [ ] Review the month's logged findings and identify one recurring theme.
- [ ] Update the mistake log if the postmortem reveals a misconception you held.

### Finding Log Format

Each entry: date, source, one-line summary, vulnerability/failure class, control that would have prevented it, link.

### Recall Checks

- Why does a security engineer who stops reading the field fall behind faster than in most other technical disciplines?
- What is the difference between a CVE and an exploited CVE, and why does that distinction matter for prioritization?
- Why are postmortems higher-value reading than generic security blog content?
- How does a finding log differ from a mistake log?

### Exit Criteria

- [ ] Daily read system defined and run for at least one week.
- [ ] Weekly postmortem review run for at least two cycles.
- [ ] Finding log created with at least two real entries.
- [ ] At least one entry cross-referenced into the mistake log.

---

## 13. Phase 1 Capstone — Foundation Security Toolkit
...
## 14. Recall and Neuroscience Plan
...
- [ ] Security research habit system running for at least one week.
...
- [ ] HackTheBox Starting Point rooms.
- [ ] TryHackMe Linux, Networking, and Web fundamentals paths.